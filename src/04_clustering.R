...
#   → σ par médiane des distances, α par silhouette
#
# ÉVALUATION :
...
cat(sprintf("    Meilleur α = %.2f, ω = %.2f (silhouette)\n", alpha_best_B, omega_best_B))
cat(sprintf("    Silhouette = %.3f | ARI = %.3f\n",
...
resultats_B <- list(
  ...
  omega = omega_best_B,
  grid = grille_B,
  methode = sprintf("PAM sur Dw (α=%.2f, ω=%.2f)", alpha_best_B, omega_best_B)
)
...
resultats_C <- list(
  ...
  alpha = alpha_best_C,
  grid = grille_C,
  methode = sprintf("PAM sur DK (α=%.2f)", alpha_best_C)
)
...
# ═══════════════════════════════════════════════════════════════════════════
# CHAÎNE HFV + NOYAUX HYBRIDES SUR RECONSTRUCTIONS (02b → 03b)
# ═══════════════════════════════════════════════════════════════════════════
cat("\n  ── Chaîne HFV + DK (courbes reconstruites, noyau hybride) ──\n")
source("src/02b_pca_hybride_reconstruction.R")
source("src/03b_distances_noyaux_hybrides.R")
pam_DK_hfv <- pam(as.dist(D_K), k = n_clusters, diss = TRUE)
sil_DK_hfv <- pam_DK_hfv$silinfo$avg.width
ari_DK_hfv <- adjustedRandIndex(pam_DK_hfv$clustering, labels_vrai)
cat(sprintf("    HFV+DK   : Silhouette = %.3f | ARI = %.3f (r = %.4f)\n",
            sil_DK_hfv, ari_DK_hfv, r))

resultats_DK_hfv <- list(
  labels = pam_DK_hfv$clustering,
  silhouette = sil_DK_hfv,
  ari = ari_DK_hfv,
  methode = "HFV + DK (reconstr.)"
)
...
comparaison <- data.frame(
  Strategie  = c("Baseline D0", "Baseline Ds",
                  "A (FPCA+Z)",
                  sprintf("B (Dw α=%.2f ω=%.2f)", alpha_best_B, omega_best_B),
                  sprintf("C (DK α=%.2f)", alpha_best_C),
                  "HFV + DK (reconstr.)"),
  Silhouette = round(c(sil_D0, sil_Ds, sil_A,
                        resultats_B$silhouette, sil_C, sil_DK_hfv), 3),
  ARI        = round(c(ari_D0, ari_Ds, ari_A,
                        resultats_B$ari, ari_C, ari_DK_hfv), 3)
)
...
if (Sys.getenv("CNAM_EXPORT_COMPARAISON", "") == "1") {
  ...
  write.csv(comparaison, fn, row.names = FALSE)
