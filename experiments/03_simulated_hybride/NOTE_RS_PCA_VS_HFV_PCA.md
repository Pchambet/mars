...
- **`R`** (notation du rapport ; **R** comme *ratio*) : rapport **V_F / V_Y** dans l’ACP hybride (etape 02b) — pondération
  **fonctionnel / vectoriel entre blocs** avant fusion dans `chi`.
  Ce n’est **pas** le meme objet que **ω** (distance Dw). Le code peut nommer la variable **`r`**, mais le mémoire utilise **`R`** pour éviter toute confusion avec **`ω`**.
...
- **R** = V_F / V_Y (souvent `r` dans le code) ; **γ_pond** = √R · **γ** ; **χ** = [**η** | **γ_pond**].
...
- On pose un espace hybride fonctionnel + vectoriel avec un produit scalaire
  pondere.
- Le parametre de pondération inter-blocs HFV est explicite (**`R`** dans le rapport,
  souvent **`r`** dans `02b`, calcule par **V_F / V_Y** avec **V_Y** sur les scores **γ** de la PCA vectorielle).
...
- `02b_pca_hybride_reconstruction.R` :
  ...
  - calcule **`r = V_F / V_Y`** (noté **R** dans le mémoire) et construit **`chi`**,
...
Conclusion : la chaine est une implementation pratique **HFV** (double ACP + fusion + reconstruction), comparable en esprit a une logique RS par concatenation puis reduction, mais avec **pondération explicite** **R** (variable **`r`** dans le script) entre blocs fonctionnel et vectoriel (scores **γ**, pas colonnes brutes de **Z_std** seules).
