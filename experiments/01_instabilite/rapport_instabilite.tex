...
$(\alpha,\omega)$ sur trois jeux de données ; les labels ne servent qu'à
...
Trois jeux de données (labels uniquement pour \textbf{évaluer} après coup) :
Canadian Weather, Growth, Tecator. Les $k_{\mathrm{vrai}}$ utilisés dans les
tableaux viennent des scripts \texttt{src/00\_preprocess*.R} : Canadian
(\texttt{CanadianWeather\$region}), Growth (garçons/filles), Tecator
via des règles définies dans le projet (seuils sur la matière grasse)
...
$\to 4$, Growth $\to 2$, Tecator $\to 3$, cohérent avec les
$k_{\mathrm{vrai}}$ retenus pour l'évaluation (sauf conventions sur Tecator).
...
le vrai $k$ est connu ; elles ne remplacent pas l'étude sur les trois jeux réels.
