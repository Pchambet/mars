...
évaluer l'ARI une fois $k$ choisi. Un \textbf{volet simulé} ajoute des données
artificielles où le vrai $k$ est connu : l'idée est de voir si la méthode se
comporte bien quand on connaît la réponse. Pour gagner du temps, ce volet utilise
...
\bottomrule
\end{tabular}
\caption{Volet simulé : grille $6\times6$, $B=60$, graine~1. Un calcul aussi fin
...
\begin{itemize}
  ...
  \item \textbf{Volet simulé (réglage utilisé dans ce rapport) :} grille
...
de $(k,\alpha,\omega)$. Le volet simulé illustre la méthode quand le vrai $k$ est
connu ; il ne remplace pas l'étude sur les quatre jeux réels.
